...
class Cliente {
    ...
    function __construct($dni,$nombre,$apellidos,$fnac,$email,$dir) {
        $this->dni = $dni;
        $this->nombre = $nombre;
    ...
    function darAlta($conn) {
        ...
        $sql = "INSERT INTO Clientes (dni,nombre,apellidos,fnacimiento,email,direccion) VALUES ('".$this->dni."','".$this->nombre."','".$this->apellidos."','".$this->fnacimiento."','".$this->email."','".$this->direccion."');";
    ...
    function buscar($busqueda,$tipoBusqueda,$conn) {
      ...
      if ($resultado->num_rows > 0) {
        ...
        while($row = $resultado->fetch_assoc()) {
          echo "- Nombre: ".$row["nombre"].", Apellidos: ".$row["apellidos"].", Email: ".$row["email"].", DNI: ".$row["dni"].", Dirección: ".$row["direccion"]."<br>";
        }
      }else{
